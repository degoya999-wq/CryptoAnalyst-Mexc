...
# ═══════════════════════════════════════════════════════════════════════
# v7.3.8: MALLOC_TRIM — возврат памяти из glibc-пулов обратно в ОС
# Python/numpy/ccxt освобождают объекты, но glibc malloc держит память
# в своих пулах "на будущее" — RSS процесса не уменьшается. malloc_trim(0)
# заставляет glibc вернуть unused память операционной системе.
# Это КЛЮЧЕВОЙ фикс против OOM на Render Free (512MB): после gc.collect
# Python считает память свободной, но ОС видит её занятой — trim это чинит.
# Работает только на Linux с glibc (Render = Ubuntu, ОК). На других — no-op.
# ═══════════════════════════════════════════════════════════════════════
import ctypes
import ctypes.util
...
try:
    ...
    logging.warning(f"malloc_trim недоступен (не glibc?): {_e}")


def _malloc_trim(tag: str = ""):
    """Возвращает unused память из glibc malloc-пулов в ОС.
    Вызывать после gc.collect() для максимального эффекта."""
    if _libc is None:
        return
    try:
        ret = _libc.malloc_trim(0)
        if tag:
            logging.info(f"🧹 malloc_trim[{tag}]: returned={ret}")
    except Exception as e:
        logging.warning(f"malloc_trim[{tag}] failed: {e}")
...
CANDLES_LIMIT_4H = 99    # для ATR Map baseline 50 + REVERSAL lookback 10 + запас
CANDLES_LIMIT_2H = 80    # для ATR Map baseline 50 + EMA20 + запас
CANDLES_LIMIT_1H = 30    # для reversal 1H триггера

candles_storage = {
    '4h': {},   # {symbol: deque(maxlen=CANDLES_LIMIT_4H)}
    '2h': {},   # {symbol: deque(maxlen=CANDLES_LIMIT_2H)}
    '1h': {},   # {symbol: deque(maxlen=CANDLES_LIMIT_1H)} — только для high-vol reversal
}
...
# v7.3.8: время последнего бана — для smart warmup.
# Если IP недавно был забанен, warmup идёт медленнее, чтобы не словить
# повторный бан на "горячем" IP (watchlist Binance держится часами).
LAST_BAN_AT = 0.0
...
def set_ip_ban_until(ban_ts_sec: float):
    ...
    global IP_BAN_UNTIL, LAST_BAN_AT
    with _IP_BAN_LOCK:
        if ban_ts_sec > IP_BAN_UNTIL:
            IP_BAN_UNTIL = ban_ts_sec
            LAST_BAN_AT = time.time()   # v7.3.8: запоминаем когда поймали бан
            logging.warning(
...
def set_candles(symbol: str, timeframe: str, ohlcv_list):
    ...
    if timeframe == '4h':
        maxlen = CANDLES_LIMIT_4H
    elif timeframe == '2h':
        maxlen = CANDLES_LIMIT_2H
    elif timeframe == '1h':
        maxlen = CANDLES_LIMIT_1H
    else:
        maxlen = 100
    with candles_lock:
...
def get_market_context():
    """v7.3: контекст рынка по BTC/ETH 4H. Берём из candles_storage если есть,
    иначе fallback на REST (только до прогрева)."""
    # Сначала пробуем взять из хранилища (после прогрева так и должно быть)
    btc = get_candles('BTC/USDT:USDT', '4h')
    eth = get_candles('ETH/USDT:USDT', '4h')
    ...
    for attempt in range(3):
        try:
            btc_rest = safe_api_call(exchange.fetch_ohlcv, 'BTC/USDT:USDT', '4h', limit=5)
            if btc_rest is None:
                time.sleep(5); continue
            btc_ch = ((btc_rest[-1][4] - btc_rest[-2][4]) / btc_rest[-2][4]) * 100
            eth_rest = safe_api_call(exchange.fetch_ohlcv, 'ETH/USDT:USDT', '4h', limit=5)
            if eth_rest is None:
                time.sleep(5); continue
...
def fetch_all_funding_rates(exchange):
    """
    Загружает funding rate для ВСЕХ USDT-perp одним запросом (weight ~5).
    v7.3: через safe_api_call (учитывает global ban flag).
    """
...
def _warmup_pause_sec() -> float:
    """v7.3.8: возвращает паузу между парами warmup-запросов в зависимости
    от истории банов. Цель — не словить повторный бан на горячем IP.

    Логика:
      - бан был <10 мин назад → 0.8с пауза (медленный, бережём IP)
      - бан был <2 часов назад → 0.3с пауза (осторожный)
      - бана давно/не было → 0с (ccxt rate limiter сам разрулит, как раньше)

    На горячем IP warmup растянется с ~6 до ~15-25 минут, но это НАМНОГО
    безопаснее чем повторный бан который эскалирует watchlist.
    """
    if LAST_BAN_AT <= 0:
        return 0.0
    since_ban = time.time() - LAST_BAN_AT
    if since_ban < 600:        # <10 минут
        return 0.8
    elif since_ban < 7200:     # <2 часов
        return 0.3
    return 0.0
...
def warmup_history():
    ...
    total = len(syms) * 2  # 4H + 2H для каждой монеты
    warmup_state['total'] = total
    warmup_state['done'] = 0

    logging.info(f"🔄 WARMUP START: {len(syms)} монет × 2 таймфрейма = {total} запросов")
    # v7.3.8: проверяем не горячий ли IP (недавний бан) — если да, warmup медленнее
    _wp = _warmup_pause_sec()
    if _wp > 0:
        since = time.time() - LAST_BAN_AT
        logging.warning(
            f"⏳ WARMUP SAFE MODE: IP был забанен {since:.0f}с назад, "
            f"пауза {_wp}с между монетами (warmup будет медленнее ради защиты IP)"
        )
    _log_memory("warmup_start")

    for i, sym in enumerate(syms):
        # 4H
        ohlcv_4h = fetch_ohlcv_with_retry(sym, '4h', limit=CANDLES_LIMIT_4H, max_retries=2)
        if ohlcv_4h and len(ohlcv_4h) >= 60:
            set_candles(sym, '4h', ohlcv_4h)
        else:
        ...
        # v7.3.8: smart-пауза между монетами на горячем IP.
        if _wp > 0:
            time.sleep(_wp)
        # КРИТИЧНО: переоцениваем режим КАЖДЫЕ 50 монет ВСЕГДА (не только если
        # пауза уже活на). Иначе сценарий: warmup стартовал без бана (_wp=0),
        # на 500-й монете прилетел бан — без этой переоценки мы бы продолжили
        # долбить с нулевой паузой и эскалировали бан. Теперь подхватим паузу.
        if (i + 1) % 50 == 0:
            _wp = _warmup_pause_sec()
            if _wp > 0:
                since = time.time() - LAST_BAN_AT
                logging.warning(
                    f"⏳ WARMUP SAFE MODE активирован на монете {i+1}: "
                    f"IP забанен {since:.0f}с назад, пауза {_wp}с"
                )
    ...
    logging.info(
        ...
        f"errors={warmup_state['errors']}, candles_4h={stats.get('4h',0)}, "
    ...
    logging.info(f"🧹 gc.collect после warmup: освобождено {collected} объектов")
    _malloc_trim("after_warmup")
...
ws_status = {
    ...
    'last_forced_reconnect': 0.0,  # v7.3.8: когда последний раз форсили reconnect (rate-limit)
    'forced_reconnects':     0,    # v7.3.8: счётчик форс-реконнектов (для /health)
}

# v7.3.8: STALE-RECOVERY параметры.
# Вчерашний бан случился так: WS-свечи "полузависли" (соединение открыто,
# данные не идут, исключения нет) на 35 минут. Аналитика работала на старых
# свечах, а reconnect-цикл крутился вхолостую. Нужен активный detector:
# если свечи молчат дольше порога — пересоздать event loop.
# КРИТИЧНО для анти-бана: реконнект НЕ чаще раза в N минут (иначе лавина
# реконнектов сама вызовет бан) и НЕ во время активного бана.
WS_STALE_THRESHOLD_SEC      = 300   # 5 мин без свечей = пора пересоздавать WS
WS_FORCED_RECONNECT_MIN_GAP = 600   # не чаще раза в 10 минут (анти-флуд)
            ...
            while True:
                ...
                if tickers_update:
                    with ws_tickers_lock:
                        ...
                        # пары которых нет в этом тике остаются с прошлыми значениями.
...
async def _ws_stale_monitor():
    """v7.3.8: следит за свежестью WS-свечей. Если данные не приходят дольше
    WS_STALE_THRESHOLD_SEC — поднимает CancelledError на всю _ws_main группу,
    что заставляет ws_loop пересоздать event loop (полный реконнект).

    Три предохранителя против лавины реконнектов (которая сама = бан):
      1. Срабатывает только при stale > 5 минут.
      2. Не чаще раза в WS_FORCED_RECONNECT_MIN_GAP (10 мин).
      3. Не трогает WS во время активного бана (ждём естественного снятия).

    Возвращает (выходит из await), когда обнаружил stale и пора реконнектиться.
    Это штатный выход — _ws_main увидит что монитор завершился и отменит остальных.
    """
    # Даём время на первичную подписку — сразу после старта свечей ещё нет.
    await asyncio.sleep(WS_STALE_THRESHOLD_SEC)
    while True:
        await asyncio.sleep(30)

        # Предохранитель 3: во время бана ничего не делаем.
        with _IP_BAN_LOCK:
            ban_active = IP_BAN_UNTIL > time.time()
        if ban_active:
            continue

        last_msg = ws_status.get('last_message_at', 0.0)
        if last_msg <= 0:
            continue  # ещё ни одного сообщения не было — рано судить
        stale_sec = time.time() - last_msg
        if stale_sec < WS_STALE_THRESHOLD_SEC:
            continue  # всё свежо

        # Предохранитель 2: rate-limit на форс-реконнект.
        since_last_forced = time.time() - ws_status.get('last_forced_reconnect', 0.0)
        if since_last_forced < WS_FORCED_RECONNECT_MIN_GAP:
            # Stale есть, но мы недавно уже реконнектились — ждём, не флудим.
            logging.warning(
                f"⚠️ WS stale {stale_sec:.0f}s, но форс-реконнект был "
                f"{since_last_forced:.0f}s назад (<{WS_FORCED_RECONNECT_MIN_GAP}s) — жду"
            )
            continue

        # Все предохранители пройдены — инициируем пересоздание WS.
        ws_status['last_forced_reconnect'] = time.time()
        ws_status['forced_reconnects'] += 1
        logging.warning(
            f"🔄 WS STALE RECOVERY: свечи молчат {stale_sec:.0f}s "
            f"(>{WS_STALE_THRESHOLD_SEC}s) → пересоздаю event loop "
            f"(форс-реконнект #{ws_status['forced_reconnects']})"
        )
        return  # выход → _ws_main отменит watchers и ws_loop пересоздаст loop
...
async def _ws_main():
    ...
    v7.3.8: + _ws_stale_monitor — если свечи зависли, пересоздаём loop."""
    logging.info("📡 WS main: starting 3 timeframe watchers + tickers watcher + stale monitor")
    ...
    #
    # v7.3.8: stale-monitor запускается как ОТДЕЛЬНАЯ задача. Когда он
    # завершится (обнаружив stale), мы отменяем все watcher-задачи и выходим —
    # ws_loop пересоздаст event loop с нуля. Watchers обёрнуты в задачи, чтобы
    # их можно было явно отменить.
    watcher_tasks = [
        asyncio.ensure_future(_ws_watch_tickers()),
        asyncio.ensure_future(_ws_watch_timeframe(ws_exchanges['4h'], '4h')),
        asyncio.ensure_future(_ws_watch_with_delay(ws_exchanges['2h'], '2h', delay=30)),
        asyncio.ensure_future(_ws_watch_with_delay(ws_exchanges['1h'], '1h', delay=60)),
    ]
    monitor_task = asyncio.ensure_future(_ws_stale_monitor())

    # Ждём пока ЛИБО монитор скажет "stale, реконнект", ЛИБО все watchers умрут.
    done, pending = await asyncio.wait(
        [monitor_task, *watcher_tasks],
        return_when=asyncio.FIRST_COMPLETED,
    )

    # Отменяем всё что ещё живо (watchers + монитор) — чистое пересоздание.
    for t in pending:
        t.cancel()
    # Дожидаемся отмены, гасим исключения чтобы не было "never retrieved".
    if pending:
        await asyncio.gather(*pending, return_exceptions=True)
    # Читаем результаты завершившихся (тоже гасим исключения).
    for t in done:
        try:
            _ = t.result()
        except Exception as e:
            logging.debug(f"_ws_main: task завершилась с {type(e).__name__}: {str(e)[:120]}")

    # v7.3.8 КРИТИЧНО: закрываем ccxt.pro WS-клиенты ПЕРЕД пересозданием loop.
    # ws_exchanges/ws_tickers_exchange — глобальные, переиспользуются между
    # event loop'ами. Их внутренние WebSocket-клиенты привязаны к ТЕКУЩЕМУ loop.
    # Если не закрыть — при следующем watch_* в новом loop будет "Event loop is
    # closed" + висящие сокеты в памяти (утечка). close() сбрасывает WS-клиентов,
    # при следующем watch_* ccxt.pro создаст их заново в новом loop.
    for tf, ws_ex in ws_exchanges.items():
        try:
            await ws_ex.close()
        except Exception as e:
            logging.debug(f"_ws_main: close ws_exchanges[{tf}]: {type(e).__name__}: {str(e)[:80]}")
    try:
        await ws_tickers_exchange.close()
    except Exception as e:
        logging.debug(f"_ws_main: close ws_tickers_exchange: {type(e).__name__}: {str(e)[:80]}")

    logging.info("📡 WS main: цикл завершён, ws_loop пересоздаст соединения")
...
def ws_loop():
    ...
    v7.3.8: при завершении _ws_main (штатном — от stale monitor, или аварийном)
    КОРРЕКТНО закрывает event loop, чтобы не копить незакрытые loop/socket
    объекты в памяти (это часть фикса утечки)."""
    while True:
        loop = None
        try:
            ...
            # _ws_main вернулся штатно (stale recovery) — пересоздадим loop ниже.
            logging.info("📡 ws_loop: _ws_main завершился, готовлю пересоздание")
        except Exception as e:
            logging.error(f"ws_loop crashed: {type(e).__name__}: {e}")
            ws_status['connected'] = False
        finally:
            # v7.3.8: аккуратно гасим event loop — отменяем оставшиеся задачи,
            # закрываем loop. Без этого каждый реконнект оставляет "висящий"
            # loop с сокетами → утечка памяти → OOM.
            if loop is not None:
                try:
                    ...
                    logging.debug(f"ws_loop cleanup: {type(e).__name__}: {str(e)[:120]}")
                finally:
                    loop.close()
                    ws_status['connected'] = False
                    # Освобождаем память от закрытого loop перед новым циклом.
                    gc.collect()
                    _malloc_trim("ws_reconnect")
        # Небольшая пауза перед пересозданием — не крутимся вхолостую.
        time.sleep(10)
...
def analyst_loop():
    ...
    logging.info("Аналитик Binance v7.3.3 (WebSocket) запущен.")
        ...
        try:
            ...
            for symbol in symbols:
                try:
                    ...
                    # 4H блок: v7.3 — свечи берём из candles_storage (заполняется WebSocket).
                    # При старте после прогрева — у всех монет есть CANDLES_LIMIT_4H=99 свечей.
                    # Дальше WS дописывает свежие свечи каждые 4 часа.
                    # Если для этой монеты ещё нет данных (только что появилась) — skip.
                    ohlcv = get_candles(symbol, '4h')
                    ...
                    if (need_rev_long or need_rev_short) and vol_24h >= MIN_VOLUME_SIGNAL:
                        # v7.3: 1H из candles_storage (WebSocket)
                        ohlcv_1h_rev = get_candles(symbol, '1h')
            ...
            # v7.3.8: освобождаем память после итерации.
            # analyst_loop создаёт сотни временных numpy-массивов на 579 монет —
            # gc.collect собирает их, malloc_trim возвращает память ОС.
            # Это главный фикс дрейфа памяти (было +160MB/2ч → цель +50MB/2ч).
            _iter_count = bot_status["iterations"]
            collected = gc.collect()
            _malloc_trim()
            # Раз в 10 итераций (~час) логируем эффект для мониторинга
            if _iter_count % 10 == 0:
                logging.info(f"🧹 gc после итерации #{_iter_count}: освобождено {collected} объектов")
                _malloc_trim(f"iter_{_iter_count}")
                _log_memory(f"iter_{_iter_count}")
...
def health():
    ...
    # v7.3.8: показываем сколько раз сработал stale-recovery
    if ws_status.get('forced_reconnects', 0) > 0:
        ws_str += f" | stale-recovery: {ws_status['forced_reconnects']}×"

    # Candles в памяти
    cs = candles_stats()
    candles_str = f"4h={cs.get('4h',0)} | 2h={cs.get('2h',0)} | 1h={cs.get('1h',0)}"
    ...
    return (f"✅ OK | Binance v7.3.8 (WS-tickers + stale-recovery)\n"
            f"Uptime: {uptime}\n"
            ...
            f"🔔 ВНИМАНИЕ 4H: {bot_status['attention_sent']}\n"
...
def startup_sequence():
    ...
    while True:
        try:
            logging.info("🚀 Startup v7.3.8: load_markets() (с retry + heartbeat)...")
            _safe_load_markets()
            ...
            try:
                ...
                logging.info(
                    f"✅ v7.3.6: markets shared между REST + 3 ws_exchanges + ws_tickers "
...
def start_bot():
    ...
    logging.info("🚀 start_bot v7.3.8: все фоновые потоки запущены")
