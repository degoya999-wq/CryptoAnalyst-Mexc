...
# ─────────────────────────────────────────────
# Binance v7.3.4: фикс memory leak от "Future exception was never retrieved"
# - Баг v7.3.3: создавалось 12 batch-задач × 3 таймфрейма = 36 async tasks.
#   При обрывах WS (NetworkError code 1008) некоторые batch-tasks падали
#   с exception, который "не собирался" корректно через gather() →
#   zombie-tasks накапливались в памяти каждый час → Render видел превышение
#   512MB → перезапускал контейнер каждые 1-4 часа → 9 минут провала по алертам.
# - Фикс v7.3.4: упрощённая WS-логика — 1 task на таймфрейм вместо 12.
#   ВСЕ монеты одного таймфрейма подписываются ОДНИМ вызовом
#   watch_ohlcv_for_symbols (ccxt.pro это поддерживает штатно).
#   3 параллельных async tasks вместо 36 → память ~250-300MB вместо ~500MB
#   → больше нет рестартов от Render по memory limit.
# - Разнесение старта watcher'ов по времени (4h сразу, 2h через 30с, 1h через 60с)
#   ОСТАВЛЕНО — это работающая защита от flood при первом подключении.
# - Логика ATR Map / is_sq / mature_bars / пороги / сортировка — НЕ ИЗМЕНЕНА.
#
...
# WebSocket exchange (ccxt.pro) — для подписки на свечи через watch_ohlcv_for_symbols.
# Это отдельный экземпляр, работает в asyncio event loop в отдельном потоке.
...
def refresh_active_symbols(initial: bool = False):
    ...
    try:
        ...
        if tickers is None:
            logging.warning("refresh_active_symbols: fetch_tickers returned None")
            return False
...
async def _ws_watch_timeframe(ws_ex, timeframe: str):
    """v7.3.4: упрощённый watcher — ОДИН вызов watch_ohlcv_for_symbols на ВСЕ монеты.

    Стратегия изменена с v7.3.3:
      - Раньше: создавалось 12 batch-задач по 50 монет → 36 параллельных async tasks
                на 3 таймфрейма → memory leak от "Future exception was never retrieved"
                → Render рестартил контейнер каждые 1-4 часа.
      - Теперь: 1 task на таймфрейм с полным списком монет в одном вызове.
                ccxt.pro сам обрабатывает 579 streams в одном combined stream.
                Параллельных tasks стало 3 вместо 36 → память упала с ~500MB до ~250MB.

    При обрыве/ошибке этот же task сам пересоздаёт подписку с exponential backoff.
    Никаких отдельных batch tasks — нет источника zombie-задач.
    """
    ...
    while True:
        try:
            # Проверяем глобальный бан перед каждым подключением
            ...
            # ОДИН вызов watch_ohlcv_for_symbols на ВСЕ монеты данного таймфрейма
            sym_tf_pairs = [[s, timeframe] for s in syms]
            logging.info(
                f"📡 ws_watch[{timeframe}]: subscribing to {len(syms)} symbols (single subscription)"
            )
            ws_status['batch_progress'] = f"{timeframe} (1 stream × {len(syms)})"

            # Внутренний цикл — пока подписка работает, получаем обновления
            while True:
                try:
                    ...
                    # Ошибка внутри подписки — выкидываем наверх для переподключения
                    raise inner_e
            ...
            # Логируем 3 первые ошибки подряд, потом каждую 10-ю (без флуда)
            if consecutive_errors <= 3 or consecutive_errors % 10 == 0:
                logging.warning(
                    f"ws_watch[{timeframe}] error #{consecutive_errors}: "
                    f"{err_type}: {err_str[:200]}"
                )
            # Распознаём бан в ошибке
...
async def _ws_main():
    """v7.3.4: главная корутина. Запускает 3 task — по одному на таймфрейм.
    4h стартует сразу, 2h через 30с, 1h через 60с — разнесение по времени
    предотвращает flood при первом подключении."""
    logging.info("📡 WS main: starting 3 timeframe watchers (4h, 2h delayed 30s, 1h delayed 60s)")
    # 3 async task вместо 36 (как было в v7.3.3 с батчами)
    await asyncio.gather(
        _ws_watch_timeframe(ws_exchange, '4h'),
        _ws_watch_with_delay(ws_exchange, '2h', delay=30),
        _ws_watch_with_delay(ws_exchange, '1h', delay=60),
        return_exceptions=True,
    )


async def _ws_watch_with_delay(ws_ex, timeframe: str, delay: int):
    """Запускает watcher после задержки. Разносит первое подключение по времени,
    чтобы Binance не получил единовременный пик коннектов."""
...
def analyst_loop():
    ...
    logging.info("Аналитик Binance v7.3.4 (WebSocket) запущен.")
    ...
    while True:
        try:
            ...
            # v7.3: fetch_tickers — REST, защищён safe_api_call
            tickers = safe_api_call(exchange.fetch_tickers)
            if tickers is None:
...
def health():
    ...
    return (f"✅ OK | Binance v7.3.4 (WebSocket)\n"
            f"Uptime: {uptime}\n"
...
def startup_sequence():
    ...
    while True:
        try:
            logging.info("🚀 Startup v7.3.4: load_markets() (с retry + heartbeat)...")
            _safe_load_markets()
