...
def refresh_active_symbols(initial: bool = False):
    ...
    try:
        # load_markets для гарантии актуальности (раз в час, weight копеечный)
...
def analyst_loop():
    ...
    logging.info("Аналитик Binance v7.3 (WebSocket) запущен.")
...
def health():
    ...
    return (f"✅ OK | Binance v7.3 (WebSocket)\n"
            f"Uptime: {uptime}\n"
...
def startup_sequence():
    """1) загружаем рынки → 2) получаем список монет → 3) прогрев истории →
    4) запускаем analyst + ws. Запускается в отдельном потоке, чтобы Flask
    мог отвечать /health сразу (Render проверяет ping при старте)."""
    try:
        logging.info("🚀 Startup v7.3: load_markets()...")
        ...
        if not refresh_active_symbols(initial=True):
            logging.error("Startup: refresh_active_symbols failed, retry в 30с")
            time.sleep(30)
            refresh_active_symbols(initial=True)

        logging.info("🚀 Startup: запуск warmup_history (5-7 минут)...")
        warmup_history()
        logging.info("🚀 Startup: warmup done, all systems go")
    except Exception as e:
        logging.error(f"startup_sequence FATAL: {e}", exc_info=True)
