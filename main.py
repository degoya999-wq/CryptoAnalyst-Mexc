...
# - НОВОЕ: helper fetch_ohlcv_with_retry с обработкой 418/429/Network
#   Раньше 418 пропускались наверх и монета теряла алерт.
#   Теперь — retry до 2 раз с правильной паузой (30с для 418, 10с для 429).
...
exchange = ccxt.binance({
    ...
    'rateLimit': 200,                       # 200мс пауза между запросами (Binance v7.1, повышен с 100мс)
                                             # На 100мс получали ~100 банов 418 за 5 часов работы.
                                             # 200мс = в 2 раза реже запросы → в 4 раза меньше банов
                                             # (нагрузка квадратично связана с пиками).
                                             # Нагрузка ~5 запросов/сек = 300 weight/мин на устойчивом,
                                             # лимит Binance 2400/мин → запас 8х.
                                             # Итерация: ~9 мин → ~10-11 мин (приемлемо).
    'timeout': 15000,
    'options': {
...
def fetch_ohlcv_with_retry(symbol: str, timeframe: str, limit: int,
                             max_retries: int = 2):
    """
    Запрос свечей с обработкой 418/429/NetworkError (Binance v7.1).
    
    418 (IP banned) — Binance вернул код "Way too many requests; IP banned until ...".
                       Ждём 30 секунд и пробуем снова. Чаще всего бан короткий (15-60с).
    429 (Rate limit) — flood control, ждём 10 секунд.
    NetworkError    — сетевая ошибка, короткая пауза 3 секунды.
    
    Возвращает ohlcv список или None если не удалось получить после всех retry.
    
    Это решает проблему которая была в v7.1 до фикса: NetworkError ловил только
    сетевые ошибки, а 418/429 пропускались наверх и монета пропускалась полностью.
    """
    for attempt in range(max_retries + 1):
        try:
            ...
            if '418' in err_str or 'banned' in err_str.lower():
                wait = 30
                logging.warning(f"retry {symbol}: binance 418 IP banned, waiting {wait}s "
                                f"(attempt {attempt+1}/{max_retries+1})")
            else:
                wait = 10
                logging.warning(f"retry {symbol}: binance 429 rate limit, waiting {wait}s "
                                f"(attempt {attempt+1}/{max_retries+1})")
            if attempt < max_retries:
                time.sleep(wait)
            else:
                logging.error(f"retry {symbol}: исчерпаны попытки после 418/429")
                return None
        except ccxt.ExchangeNotAvailable as e:
            # Также может прилететь как ExchangeNotAvailable
            err_str = str(e)
            wait = 30 if '418' in err_str else 10
            logging.warning(f"retry {symbol}: ExchangeNotAvailable, waiting {wait}s "
                            f"(attempt {attempt+1}/{max_retries+1})")
            if attempt < max_retries:
                time.sleep(wait)
            else:
...
def fetch_all_funding_rates(exchange):
    ...
    try:
        ...
        logging.warning(f"fetch_funding_rates failed: {e}")
        return {}
        ...
        try:
            ...
            for symbol in symbols:
                try:
                    ...
                    # 4H блок: limit=100 → weight 1 (вместо weight 2 для limit>100).
                    # Экономия ~600 weight за итерацию на 577 монет.
                    # 100 свечей 4H = 16 дней истории — достаточно для ATR Map (baseline 50)
                    # и REVERSAL (lookback 10) с запасом 20 свечей.
                    # Binance v7.1 фикс: используем helper с retry на 418/429/Network.
                    ohlcv = fetch_ohlcv_with_retry(symbol, '4h', limit=100, max_retries=2)
