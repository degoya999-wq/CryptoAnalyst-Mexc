...
async def _ws_watch_timeframe(ws_ex, timeframe: str):
    """Корутина для подписки на один таймфрейм по всем активным монетам.
    Использует watch_ohlcv_for_symbols. При обрыве переподключается."""
    while True:
        try:
            # Берём актуальный список монет каждый раз (на случай если refresh обновил)
            with active_symbols_lock:
                syms = list(active_symbols)
            ...
            # Формируем список [(symbol, timeframe), ...]
            sym_tf_pairs = [[s, timeframe] for s in syms]

            logging.info(f"📡 ws_watch[{timeframe}]: subscribing to {len(syms)} symbols")

            # watch_ohlcv_for_symbols возвращает обновления для всех символов
            # в бесконечном цикле
            while True:
                ohlcv_update = await ws_ex.watch_ohlcv_for_symbols(sym_tf_pairs)
                # ohlcv_update = {symbol: {timeframe: [последняя свеча]}}
                # Распаковываем и обновляем хранилище
            ...
            logging.error(f"ws_watch[{timeframe}] error: {type(e).__name__}: {str(e)[:200]}")
            await asyncio.sleep(5)
...
async def _ws_main():
    ...
    logging.info("📡 WS main: starting 3 timeframe watchers (4h, 2h, 1h)")
    await asyncio.gather(
        _ws_watch_timeframe(ws_exchange, '4h'),
        _ws_watch_timeframe(ws_exchange, '2h'),
        _ws_watch_timeframe(ws_exchange, '1h'),
...
def analyst_loop():
    ...
    logging.info("Аналитик Binance v7.3.1 (WebSocket) запущен.")
...
def health():
    ...
    if ws_status['connected'] and ws_age >= 0 and ws_age < 120:
        ...
        ws_str = f"❌ disconnected (reconnects: {ws_status['reconnects']})"

    # Candles в памяти
    cs = candles_stats()
    candles_str = f"4h={cs.get('4h',0)} | 2h={cs.get('2h',0)} | 1h={cs.get('1h',0)}"

    return (f"✅ OK | Binance v7.3.1 (WebSocket)\n"
            f"Uptime: {uptime}\n"
...
def _safe_load_markets():
    """v7.3.1: load_markets с обработкой 418/Network в бесконечном retry.
    Без этого load_markets при холодном старте на shared IP в бане
    выбрасывает DDoSProtection и валит весь startup."""
    while True:
        if not wait_if_banned():
            # длинный бан → ждём до конца + 30с запас
            with _IP_BAN_LOCK:
                ban_until = IP_BAN_UNTIL
            wait = max(ban_until - time.time(), 0) + 30
            logging.warning(f"_safe_load_markets: long ban, sleep {wait:.0f}s")
            time.sleep(wait)
            continue
        try:
            ...
            if ban_ts:
                ...
                logging.warning(
                    f"_safe_load_markets: 🚫 418 until "
                    f"{datetime.fromtimestamp(ban_ts).strftime('%H:%M:%S')}, "
                    f"sleep {wait:.0f}s"
                )
                time.sleep(wait)
            ...
            if '418' in err_str or 'banned' in err_str.lower() or "I'm a teapot" in err_str:
                ...
                logging.warning(f"_safe_load_markets: 🚫 418 (no ts), sleep 95s")
                time.sleep(95)
                continue
            if isinstance(e, (ccxt.RateLimitExceeded, ccxt.DDoSProtection,
                              ccxt.ExchangeNotAvailable, ccxt.NetworkError)):
                logging.warning(f"_safe_load_markets: {type(e).__name__}, sleep 30s: {err_str[:200]}")
                time.sleep(30)
                continue
            logging.error(f"_safe_load_markets unexpected: {type(e).__name__}: {err_str[:200]}")
            time.sleep(30)
...
def startup_sequence():
    ...
    while True:
        try:
            logging.info("🚀 Startup v7.3: load_markets() (с retry на 418)...")
            _safe_load_markets()
