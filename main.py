...
async def _ws_main():
    ...
    logging.info("📡 WS main: starting 3 timeframe watchers (4h, 2h, 1h) on separate connections")
    ...
    await asyncio.gather(
        _ws_watch_timeframe(ws_exchanges['4h'], '4h'),
        _ws_watch_with_delay(ws_exchanges['2h'], '2h', delay=30),
        _ws_watch_with_delay(ws_exchanges['1h'], '1h', delay=60),
...
def health():
    ...
    return (f"✅ OK | Binance v7.3.3 (WebSocket)\n"
            f"Uptime: {uptime}\n"
...
def startup_sequence():
    ...
    while True:
        try:
            logging.info("🚀 Startup v7.3.6: load_markets() (с retry + heartbeat)...")
            _safe_load_markets()
            ...
            try:
                ...
                logging.info(
                    f"✅ v7.3.6: markets shared между REST + 3 ws_exchanges "
                    f"({len(shared_markets)} symbols, экономия ~90MB)"
                )
...
def start_bot():
    ...
    logging.info("🚀 start_bot v7.3.6: все фоновые потоки запущены")
