...
def safe_api_call(fn, *args, **kwargs):
    """Универсальная обёртка для API вызовов, которая:
      ...
      2. Вызывает функцию.
      3. Парсит ЛЮБУЮ ошибку на наличие "banned until" или "418" в тексте
         (ccxt может бросать 418 как голый NetworkError, RateLimitExceeded,
         ExchangeNotAvailable или DDoSProtection — нельзя полагаться на тип).
    ...
    try:
        ...
        err_str = str(e)
        # v7.2: проверяем текст ошибки В ПЕРВУЮ ОЧЕРЕДЬ —
        # ccxt может бросать 418 любым типом, нельзя полагаться на классы исключений
        ban_ts = _parse_banned_until_ms(err_str)
        if ban_ts:
            # Точное время разбана из ответа Binance
            set_ip_ban_until(ban_ts)
            logging.warning(
                f"🚫 418 IP banned (parsed) until "
        ...
        if '418' in err_str or 'banned' in err_str.lower() or "I'm a teapot" in err_str:
            # 418 без точного timestamp в тексте — ставим +60 сек
            set_ip_ban_until(time.time() + 60)
            logging.warning(f"🚫 418 IP banned (no timestamp), +60s: {type(e).__name__}")
            return None
        # Дальше — реальные сетевые/rate limit ошибки без 418
        if isinstance(e, ccxt.RateLimitExceeded):
            # 429 без явного бана
            set_ip_ban_until(time.time() + 10)
            logging.warning(f"429 rate limit, +10s pause: {err_str[:200]}")
            return None
        ...
        if isinstance(e, ccxt.NetworkError):
            # Реальная сетевая ошибка (DNS/timeout/socket)
...
bot_status = {
    ...
    "reversal_sent":    0,
    "compression_candidates_last":  0,   # v7.2: сколько кандидатов СЖАТИЕ собрано в последней итерации
    "compression_total_iterations": 0,   # v7.2: сумма всех кандидатов СЖАТИЕ за всё время (для проверки активности)
}
...
def fetch_ohlcv_with_retry(symbol: str, timeframe: str, limit: int,
    ...
    2. При ЛЮБОЙ ошибке проверяем текст на "banned until <ms>" — ccxt может бросать
       418 как голый NetworkError, RateLimitExceeded, ExchangeNotAvailable или
       DDoSProtection. Полагаться на тип исключения НЕЛЬЗЯ — только на текст.
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if '418' in err_str or 'banned' in err_str.lower() or "I'm a teapot" in err_str:
                ...
                continue
            # Реальные сетевые/rate ошибки без 418
            if isinstance(e, ccxt.RateLimitExceeded):
                set_ip_ban_until(time.time() + 10)
                logging.warning(
            ...
            if isinstance(e, (ccxt.ExchangeNotAvailable, ccxt.DDoSProtection)):
                set_ip_ban_until(time.time() + 30)
                logging.warning(
                    f"retry {symbol}: {type(e).__name__} +30s "
                    f"(attempt {attempt+1}/{max_retries+1})"
                )
                continue
            if isinstance(e, ccxt.NetworkError):
                logging.warning(
                    f"retry {symbol}: NetworkError (real) "
...
def analyst_loop():
    ...
    while True:
        try:
            ...
            # v7.2: сохраняем счётчики для /health
