...
# - rateLimit 100мс между запросами (равномерное распределение, защита от пиков 429)
...
exchange = ccxt.binance({
    ...
    'rateLimit': 100,                       # 100мс пауза между запросами (Binance v7.1)
                                             # Защита от 429: при ~577 монетах × 100мс = ~58 секунд
                                             # на 2H блок, нагрузка ~10 запросов/сек = 600 weight/мин,
                                             # лимит Binance 2400/мин → запас 4х в любой момент.
                                             # 429 практически невозможны.
    'timeout': 15000,
    'options': {
            ...
            for symbol in symbols:
                try:
                    ...
                    for attempt in range(2):
                        try:
                            # limit=100 → weight 1 (вместо weight 2 для limit>100).
