...
def format_breakdown(breakdown: dict, score_up: int, score_down: int):
    ...
    parts = []
    for key in ['EMA', 'CVD', 'Range', 'BTC', 'Fund', 'OI']:
        val = breakdown.get(key)
        if val is None:
            parts.append(f"{key} —")  # OI ещё не накоплен
        elif val > 0:
...
def analyst_loop():
    ...
    logging.info("Аналитик Binance v7.0 запущен (REVERSAL_4H + ВНИМАНИЕ + 2H NEW).")
        ...
        try:
            ...
            for symbol in all_perps_2h:
                try:
                    ...
                    # ---- запись OI snapshot для будущего расчёта 24h динамики
                    oi_now_notional = get_oi_from_ticker(symbol, tickers, current_2h)
                    record_oi_snapshot(symbol, oi_now_notional)
                    oi_24h_change = get_oi_24h_change(symbol, oi_now_notional)
            ...
            for cand in compression_candidates:
                ...
                # ── Объём текущей 2H
                cur_vol_line = f"🔇 Vol текущей 2H: {cand['cur_vol_2h_x']:.2f}x"
                ...
                # ── OI
                oi_now_m = cand['oi_now'] / 1e6 if cand['oi_now'] > 0 else 0
                if cand['oi_24h_change'] is None:
                    oi_line = f"🎯 OI: ${oi_now_m:.0f}M (history loading)"
                else:
                    oi_line = f"🎯 OI: ${oi_now_m:.0f}M | 24h: {cand['oi_24h_change']:+.1f}%"
            ...
            time.sleep(60)  # Binance v7.0: пауза 60с (итерация ~6-7 мин)
